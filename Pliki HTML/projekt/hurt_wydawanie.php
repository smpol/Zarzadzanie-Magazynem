    ...
    <div class="container">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            if (isset($_POST['wydaj']))
            {
                ...
                if ($result)
                {
                    echo '<div class="alert alert-success" role="alert">Produkty zostały wydane</div>';
                }
                else
                {
                    echo '<div class="alert alert-danger" role="alert">Produkty nie zostały wydane</div>';
                }     
            }
        }
        
        ?>
        <form method="POST" action="hurt_wydawanie.php">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0">Wydaj produkty</h4>
                </div>
                <div class="card-body">
                    <h4>Wybierz produkty do wydania:</h4>
                            ...
                            <tbody id="tabelka">
                                ...
                                //add selected option to table from select2 and delete from select2
                                    $('#produkty').on('select2:select', function(e) {
                                        ...
                                        var id = data.id;
                                        var nazwa = data.text;
                                        //get ilosc fron nazwa
                                        var ilosc = nazwa.match(/\(([^)]+)\)/)[1];
                ...
                <div class="card-footer">
                    <button class="btn btn-primary" type="submit" name="wydaj">Wydaj</button>
                </div>
            </div>
        </form>
    </div>
