...
<body>
    ...
    <div class="container">
        <h1 style="text-align: center;" class="mb-xl-4 mt-xl-4">Kontrahenci</h1>
    </div>
    <div class="container">
        <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['dodaj'])) {
                $nazwa = htmlspecialchars($_POST['nazwa_kontrahenta']);
                ...
                $query = "INSERT INTO kontrahenci (nazwa, email, phone) VALUES ('$nazwa', '$email', '$phone')";
                $result = pg_query($connect, $query);
                if ($result) {
                    echo '<div class="alert alert-success" role="alert">Dodano kontrahenta <b>"' . $nazwa . '"</b></div>';
                } else {
                    echo '<div class="alert alert-danger" role="alert">Nie udało się dodać kontrahenta <b>"' . $nazwa . '"</b></div>';
                }
            } else if (isset($_POST['usun'])) {
                $id = htmlspecialchars($_POST['id']);
                $query = "DELETE FROM kontrahenci WHERE id = $id";
                $result = pg_query($connect, $query);
                if ($result) {
                    echo '<div class="alert alert-success" role="alert">Usunięto kontrahenta o id <b>"' . $id . '"</b></div>';
                } else {
                    echo '<div class="alert alert-danger" role="alert">Nie udało się usunąć kontrahenta o id <b>"' . $id . '"</b></div>';
                }
                ...
                $email = htmlspecialchars($_POST['email']);
                $phone = htmlspecialchars($_POST['phone']);
                $query = "UPDATE kontrahenci SET nazwa = '$nazwa', email = '$email', phone = '$phone' WHERE id = $id";
                $result = pg_query($connect, $query);
                if ($result) {
                    echo '<div class="alert alert-success" role="alert">Zmodyfikowano kontrahenta o id <b>"' . $id . '"</b></div>';
                } else {
                    echo '<div class="alert alert-danger" role="alert">Nie udało się zmodyfikować kontrahenta o id <b>"' . $id . '"</b></div>';
                }
            }
        ...
        ?>
        <div class="text-center"><a class="btn btn-primary" data-bs-toggle="collapse" aria-expanded="false" aria-controls="collapse-1" href="#collapse-1" role="button" style="margin-bottom: 16px;">Dodaj kontrahenta</a>
            <div class="collapse" id="collapse-1" style="padding-bottom: 29px;">

                <form method="POST" action="kontrahenci.php">
                    <div class="card">
                        <div class="card-header" style="padding-bottom: 0px;padding-top: 12px;">
                            <p class="text-start">Dodawanie Kategorii</p>
                        </div>
                        <div class="card-body">
                            <p class="text-start">Nazwa Kontrahenta</p><input name="nazwa_kontrahenta" class="form-control" type="text" style="margin-bottom: 11px;" required="">
                            <p class="text-start">Adres Email</p><input name="email" class="form-control" type="email" style="margin-bottom: 11px;">
                            <p class="text-start">Numer Telefonu</p><input name="phone" class="form-control" type="tel" style="margin-bottom: 11px;">
                            <input class="btn btn-success" type="submit" value="Dodaj" name="dodaj">
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="table-responsive ps-md-2">
            <table class="table table-striped" id="myTable">
                ...
                <tbody>
                    ...
                    include_once 'bazadanych.php';
                    $connect = connect_database();
                    $query = "SELECT id, nazwa, email, phone FROM kontrahenci";
                    $result = pg_query($connect, $query);
                    $count = pg_num_rows($result);
                    echo "<input type='hidden' id='count' value='$count'>";
                    if ($count == 0) {
                        echo "<tr><td colspan='5' style='text-align: center;'>Brak kontrahentów</td></tr>";
                    } else {
                        while ($row = pg_fetch_row($result)) {
                            ...
                            if ($row[2] == null) {
                                ...
                                echo "<td>";
                                echo "<a href='mailto:$row[2]'>$row[2]</a>";
                                echo "</td>";
                            }
                            //echo "<td>$row[3]</td>";
                            if ($row[3] == null) {
                            ...
                            echo "<input type='hidden' name='email' value='$row[2]'>";
                            echo "<input type='hidden' name='telefon' value='$row[3]'>";
                            echo "<input type='hidden' name='typ' value='kontrahenci'>";
                            ...
                            //echo "<a class='btn btn-primary' role='button' style='background: var(--bs-warning);border-style: none;margin: 1px;' href='modyfikuj_inne.html'>Modyfikuj</a></td>";
                            echo "</form>";
                            echo "</tr>";
                        }
                    }
